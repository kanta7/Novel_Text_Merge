...
```bash
# 1. リポジトリをクローン
git clone https://github.com/YOUR_USERNAME/Novel_Text_Merge.git
cd Novel_Text_Merge
...
ブラウザで `http://localhost:5000` を開き、処理するフォルダを選択して「処理を開始」をクリックします。
...
- Web UIからの処理中キャンセルはサーバー側処理を停止しません（ブラウザを閉じると表示だけ停止）
...
- [Flask](https://flask.palletsprojects.com/) — Web UI フレームワーク
