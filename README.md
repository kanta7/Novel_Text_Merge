...
- **結合オプション選択** — 「文章化 + 結合」または「文章化のみ（個別 txt 保存）」をプルダウンで選択可能
- **一括統合** — 最大 500 枚の画像を 1 つのテキストファイルに統合
- **リアルタイム進捗表示** — 文章化フェーズ・結合フェーズそれぞれの進捗を数値でブラウザにストリーミング表示
- **文中改行の結合** — 本などで発生する一文の途中での改行を取り除き、自然な一文単位に整形するオプション（ラジオボタンで選択・デフォルト有効）
...
4. **文中改行の処理** を選択（「文中の改行を結合する」推奨 / 「改行をそのまま保持する」）
5. **結合オプション** を選択（「文章化 + 結合」/ 「文章化のみ」）
6. **「処理を開始」** をクリック
7. 完了後、**「テキストファイルをダウンロード」** で結果を取得（結合ありの場合）
...
## Web UI のカスタム設定 / Web UI Options

### 文中改行の処理

| 設定 | 説明 |
|---|---|
| **文中の改行を結合する（推奨・デフォルト）** | 本などの媒体で発生する、一文の途中での折り返し改行を取り除き、文をひとつながりに整形します |
| 改行をそのまま保持する | 画像上の行区切りをそのまま再現します（原文レイアウトを忠実に保存したい場合） |

> AI がプロンプトレベルで文脈を判断するため、段落の区切りや意味的な改行は保持され、単なる折り返しのみ除去されます。

### 結合オプション

| 設定 | 説明 |
|---|---|
| **文章化 + 結合（デフォルト）** | 各画像を文章化したあと、全ページを `output/<フォルダ名>.txt` に一括統合します |
| 文章化のみ（結合しない） | 個別 txt ファイル（`output/<フォルダ名>/`）のみ保存し、統合ファイルは作成しません |

> 「文章化のみ」を選んだ場合は結合フェーズがスキップされるため処理が早く完了します。後から手動で結合することも可能です。
